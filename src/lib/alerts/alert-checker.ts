...
async function checkAlertRule(event: LogEvent, project: Project, rule: AlertRule) {
  ...
  if (rule.globalLimit?.enabled) {
    ...
    const eventsQuery = adminDb.collection('events')
    ...
    if (eventCount >= rule.globalLimit.maxAlerts) {
      ...
      const recentAlert = await checkRecentAlert(project.id, 'global', '', rule.globalLimit.windowMinutes);
  ...
  if (rule.messageRules?.length) {
    for (const messageRule of rule.messageRules) {
      ...
      if (eventCount >= messageRule.maxAlerts) {
        ...
        const recentAlert = await checkRecentAlert(project.id, 'message', messageRule.message, messageRule.windowMinutes);
...
async function checkRecentAlert(projectId: string, triggerType: 'global' | 'message', message: string, windowMinutes: number): Promise<boolean> {
...
async function createAlert(params: CreateAlertParams) {
  ...
  try {
    ...
    if (params.rule.notificationType === 'text') {
      ...
      await sendSMS(alertDoc.id, project.alertConfig.phoneNumbers, `URGENT CALL ALERT: ${params.message}`);
    }
