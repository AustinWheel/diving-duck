...
export default function EventLogsPage() {
  ...
  return (
    <Column fillWidth padding="16" gap="24" style={{ paddingTop: '32px', paddingBottom: '32px' }}>
      ...
      {/* Tabs */}
      <Flex gap="0" style={{ borderBottom: '1px solid rgba(255, 255, 255, 0.08)', paddingBottom: '16px' }}>
        <Button
              ...
              >
                <thead>
                  <tr style={{ borderBottom: '1px solid rgba(255, 255, 255, 0.08)' }}>
                    <th style={{ padding: '12px 16px', textAlign: 'left', fontWeight: 500, color: 'var(--neutral-on-background-weak)' }}>Time</th>
                    <th style={{ padding: '12px 16px', textAlign: 'left', fontWeight: 500, color: 'var(--neutral-on-background-weak)' }}>Type</th>
                    <th style={{ padding: '12px 16px', textAlign: 'left', fontWeight: 500, color: 'var(--neutral-on-background-weak)' }}>Key</th>
                    <th style={{ padding: '12px 16px', textAlign: 'left', fontWeight: 500, color: 'var(--neutral-on-background-weak)' }}>IP</th>
                    <th style={{ padding: '12px 16px', textAlign: 'left', fontWeight: 500, color: 'var(--neutral-on-background-weak)' }}>User ID</th>
                    <th style={{ padding: '12px 16px', textAlign: 'left', fontWeight: 500, color: 'var(--neutral-on-background-weak)' }}>Message</th>
                  </tr>
                </thead>
                <tbody>
                  {events.map((event, index) => (
                    ...
                    >
                      <td style={{ padding: '12px 16px', whiteSpace: 'nowrap', color: 'var(--neutral-on-background-weak)', fontSize: '12px' }}>
                      ...
                      <td style={{ padding: '12px 16px' }}>
                        <span
                          style={{
                            padding: '2px 6px',
                            borderRadius: '4px',
                            backgroundColor: event.keyType === 'prod' ? 'rgba(34, 197, 94, 0.1)' : 'rgba(251, 146, 60, 0.1)',
                      ...
                      <td style={{ padding: '12px 16px', color: 'var(--neutral-on-background-weak)', fontSize: '12px', fontFamily: 'monospace' }}>
                        {event.ip || '—'}
                      </td>
                      <td style={{ padding: '12px 16px', color: 'var(--neutral-on-background-weak)', fontSize: '12px' }}>
                        {event.meta?.userId || '—'}
                      </td>
          ...
          {selectedEvent && (
            ...
            >
              ...
              >
                ...
                <Column gap="20">
                  <Column gap="8">
                    <Text variant="body-default-xs" onBackground="neutral-weak">Timestamp</Text>
                    <Text variant="body-default-m">{new Date(selectedEvent.timestamp || '').toLocaleString()}</Text>
                  </Column>
                  
                  <Flex gap="16">
                    <Column gap="8">
                      <Text variant="body-default-xs" onBackground="neutral-weak">Type</Text>
                      <span
                        style={{
                          padding: '4px 8px',
                          borderRadius: '4px',
                    ...
                    <Column gap="8">
                      <Text variant="body-default-xs" onBackground="neutral-weak">Key Type</Text>
                      <span
                        style={{
                          padding: '4px 8px',
                          borderRadius: '4px',
                          backgroundColor: selectedEvent.keyType === 'prod' ? 'rgba(34, 197, 94, 0.1)' : 'rgba(251, 146, 60, 0.1)',
                  ...
                  <Column gap="8">
                    <Text variant="body-default-xs" onBackground="neutral-weak">Message</Text>
                    <Text variant="body-default-m" style={{ wordBreak: 'break-word' }}>{selectedEvent.message}</Text>
                  </Column>
                  
                  <Flex gap="16">
                    <Column gap="8">
                      <Text variant="body-default-xs" onBackground="neutral-weak">IP Address</Text>
                      <Text variant="body-default-m" style={{ fontFamily: 'monospace' }}>{selectedEvent.ip || '—'}</Text>
                    </Column>
                    
                    <Column gap="8">
                      <Text variant="body-default-xs" onBackground="neutral-weak">User Agent</Text>
                      <Text variant="body-default-m" style={{ fontSize: '12px', wordBreak: 'break-word' }}>
                  ...
                  {selectedEvent.meta && Object.keys(selectedEvent.meta).length > 0 && (
                    <Column gap="8">
                      <Text variant="body-default-xs" onBackground="neutral-weak">Metadata</Text>
                      <div
                        style={{
                          padding: '16px',
                  ...
                  <Column gap="8">
                    <Text variant="body-default-xs" onBackground="neutral-weak">Event ID</Text>
                    <Text variant="body-default-s" style={{ fontFamily: 'monospace', color: 'var(--neutral-on-background-weak)' }}>
                      {selectedEvent.id}
                    </Text>
                  </Column>
                </Column>
          ...
          <Column gap="16">
                  ...
                  >
                    {sandboxKeys.map(key => (
                      ...
                      >
                        ...
                        <Tag variant={key.type === 'prod' ? 'success' : 'warning'} label={key.type.toUpperCase()} />
                      </Row>
                    ))}
                  </div>
                )}
            ...
            {sandboxResult && (
              <div
                style={{
                  padding: '16px',
                  backgroundColor: sandboxResult.success ? 'rgba(34, 197, 94, 0.1)' : 'rgba(239, 68, 68, 0.1)',
...
`npm install @console-warden`,
            ...
            <CodeBlock
              ...
              codes={[
                {
                code:
`echo "WARDEN_PUBLIC_KEY=${sandboxKeys.find(key => key.id === selectedKey)?.key}" >> .env`,
                  language:"bash",
                  label:"Shell"
                }
              ]}
            ...
            <CodeBlock
              ...
              codes={[
                {
                  code:
`// Save your public key as an environment variable
// .env:
